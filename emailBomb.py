...
class Email_Bomber:
    ...
    def __init__(self):
        try:
            ...
            self.mode = int(input(bcolors.GREEN + 'Enter bomb mode(1,2,3,4) || 1:(100) 2: (250) 3: (750) 4(custom) <: '))
    ...
    def email(self):
        try:
            print(bcolors.RED + '\n{*}*{*} Setting up email {*}*{*}')
            self.server = str(input(bcolors.GREEN + 'Enter email server | or select premade optiions - 1: Gmail 2: Yahoo 3: Outlook <: '))
            premade = ['1', '2', '3']
            default_port = True
